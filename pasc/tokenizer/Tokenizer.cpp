        ...
        switch (newState) {
            ...
            case ST_BEGIN:
                switch (result.tokenType) {
                    ...
                    case TK_HEX:
                        result.text = real_src;
                        for (int i = 0; i < (int)lex_data.length(); ++i) {
                            ...
                            if (ch < 'A')
                                ...
                                ch -= 55;
                            result.intData += ch * (int)pow(16, i);
                        }
                        break;
                    case TK_CHAR:
                        result.text = real_src;
                        if (!lex_data.empty()) {
                            for (int i = 0; i < (int)lex_data.length(); ++i) {
                                ...
                                if (ch > 'F')
                                    ch -= 32;
                                result.intData += (ch - 55) * (int)pow(16, i);
                            }
                        }
                        break;
                    case TK_MUL:
...
bool Tokenizer::CheckAndNext(LEX_TYPE type)
{
	if (currentTok.tokenType == type) {
		Next();
		return true;
	}
	return false;
